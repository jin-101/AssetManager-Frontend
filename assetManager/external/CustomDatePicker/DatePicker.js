...
class DatePicker extends Component {
  ...
  _onValueChange = (key, selectedIndex) => {
    ...
    const _getSelectedIndex = (dates) =>
      selectedIndex < 0 ? 0 : Math.min(selectedIndex, dates.length - 1);
        ...
        <View style={[styles.datePickerContainer, { backgroundColor }]}>
          {dataSource.map((item, index) => {
            ...
            const initialScrollIndex =
              key === Constants.DATE_KEY_TYPE.YEAR
                ? defaultYearIndex
                : key === Constants.DATE_KEY_TYPE.MONTH
                ? defaultMonthIndex
                : defaultDayIndex;
            return (
              <DatePickerList
                ...
                onValueChange={(selectedIndex) =>
                  this._onValueChange(key, selectedIndex)
                }
                selectedRowBackgroundColor={
                  selectedRowBackgroundColor || backgroundColor
                }
                unselectedRowBackgroundColor={
                  unselectedRowBackgroundColor || backgroundColor
                }
