...
export function getDatePickerInitialData(initialProps) {
  const { type, defaultDate, minDate, maxDate, monthDisplayMode } =
    initialProps;

  const _verifyDate = (date, des) => {
    const useDefault =
      (typeof date === "string" && date.indexOf("default") > 0) ||
    ...
    const _printError = () =>
      !useDefault &&
      console.warn(
        `The date type you selected is: ${type}, but ${des} has incorrect date format: ${aDate}`
      );
    switch (type) {
      case DATE_TYPE.YYYY:
 ...
 */
export function calculateCalendarDefaultDates(dates) {
  let startDate = "";
  let endDate = "";
 ...
 */
export function toStandardStringWith(date, monthDisplayMode = "") {
