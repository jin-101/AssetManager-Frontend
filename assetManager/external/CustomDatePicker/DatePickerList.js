...
import { BORDER_LINE_POSITION, DATE_KEY_TYPE } from "./index";
...
class DatePickerList extends Component {
  ...
  constructor(props) {
    ...
    __DEV__ &&
      rows !== 5 &&
      rows !== 7 &&
      console.error("Oops! Rows is only supported by one of [5, 7]");
  ...
  _maxIndex = () => {
    const { rows, keyType, isCustom, customData } = this.props;
    ...
    const length = isCustom ? cD?.length + 4 : data.length;
  ...
  _lineStyle = (position) => {
    ...
    const { initialRow, rowHeight, selectedBorderLineMarginHorizontal } =
      this.state;
    ...
    return {
      ...
      marginLeft,
      height:
        position === BORDER_LINE_POSITION.MIDDLE
          ? rowHeight
          : initialRow * rowHeight,
      borderTopWidth:
        position === BORDER_LINE_POSITION.MIDDLE ? +selectedBorderLineWidth : 0,
      borderBottomWidth:
        position === BORDER_LINE_POSITION.MIDDLE ? +selectedBorderLineWidth : 0,
      ...
      marginTop:
        position === BORDER_LINE_POSITION.MIDDLE
          ? rowHeight * initialRow
          : position === BORDER_LINE_POSITION.TOP
          ? 0
          : (initialRow + 1) * rowHeight,
      backgroundColor:
        position === BORDER_LINE_POSITION.MIDDLE
          ? selectedRowBackgroundColor
          : unselectedRowBackgroundColor,
  ...
  _onScroll = ({ nativeEvent }) => {
    ...
    const selectedIndex =
      offsetY <= 0 ? initialRow : Math.round(offsetY / rowHeight) + initialRow;
  ...
  _renderItem = ({ item, index }) => {
    ...
    const suffix =
      keyType === DATE_KEY_TYPE.YEAR
        ? yearSuffix
        : keyType === DATE_KEY_TYPE.MONTH
        ? monthSuffix
        : daySuffix;
    return (
      <View
        style={[
          styles.datePickerListItemView,
          { height: this.state.rowHeight },
        ]}
      >
        <Text style={this._itemTextStyle(index)}>
          {item.date}
          {!item.date || suffix}
        </Text>
      </View>
  ...
  _customData = (arr, keyType) => {
    const tempData = [{ date: "" }, { date: "" }];
    const thirdData = [];
    const secondData = [];
    const fisrtData = [];
    if (keyType === "day") thirdData.push(...tempData);
    arr[2].forEach((el) => {
      thirdData.push({ date: el });
    });
    if (keyType === "day") {
      thirdData.push(...tempData);
      return thirdData;
    }

    if (keyType === "month") secondData.push(...tempData);
    arr[1].forEach((el2) => {
      secondData.push({ date: el2, data: thirdData });
    });
    if (keyType === "month") {
      secondData.push(...tempData);
      return secondData;
    }

    if (keyType === "year") fisrtData.push(...tempData);
    arr[0].forEach((el) => {
      fisrtData.push({ date: el, data: secondData });
    });
    if (keyType === "year") {
      fisrtData.push(...tempData);
      return fisrtData;
    }
  };

  render() {
    const { isCustom, customData, keyType, rows, initialScrollIndex } =
      this.props;
    ...
    return (
      <View style={{ height: heightOfContainer }}>
        ...
        <FlatList
          ...
          data={isCustom ? this._customData(customData, keyType) : data}
          initialScrollIndex={
            isCustom ? 0 : Math.min(initialScrollIndex, maxScrollIndex)
          }
